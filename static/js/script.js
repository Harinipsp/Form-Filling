...
function startVoiceInput(inputId) {
    ...
    recognition.onresult = function(event) {
        ...
        if (inputId === 'dob' || inputId === 'applicantDob') {
            ...
            const statusInputEvent = new CustomEvent('statusInput', { detail: capitalizedTranscript });
            document.dispatchEvent(statusInputEvent);
        } 
        else if (inputId==='email')
        {
            handleEmailInput(capitalizedTranscript);
        }
...
function handleEmailInput(transcript) {
    // Normalize the email input
    let normalizedEmail = transcript
    ...
    // Set the normalized email value to the email input field
    document.getElementById('email').value = normalizedEmail;

    // Optional: Log the normalized email for debugging
...
function handleGenderInput(transcript) {
    ...
    const selectedOption = genderOptions.find(
        (option) => option.toLowerCase() === normalizedTranscript
    );

    if (selectedOption) {
        genderSelect.value = selectedOption;
    } else {
        console.error("Invalid gender input:", transcript);
    }
}
document.addEventListener('statusInput', function(event) {
    const transcript = event.detail; // Get the transcript from the event
    handleStatusInput(transcript);
});

function handleStatusInput(transcript) {
    const statusSelect = document.getElementById('status');
    const statusOptions = ["Single", "Married", "Student", "Employed", "Other"];
    ...
    if (selectedOption) {
        statusSelect.value = selectedOption;
    } else {
        console.error("Invalid status input:", transcript);
    }
}
...
function handleDateOfBirthInput(transcript, inputId) {
    ...
    if (date) {
        ...
        console.error("Invalid date format:", transcript); // Log an error if the date format is invalid
    }
}
